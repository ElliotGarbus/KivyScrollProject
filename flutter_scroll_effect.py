'''
Flutter-Inspired Scroll Effect
===============================
...
LICENSE:
--------
This implementation is provided for use with Kivy (MIT License).
...
class FlutterScrollEffect(ScrollEffect):
    ...
    def __init__(self, **kwargs):
        super(FlutterScrollEffect, self).__init__(**kwargs)
        self._spring_time = 0
    ...
    def _rubber_band_clamp(self, overscroll, dimension):
        ...
        This is the key to iOS-style feel: resistance increases logarithmically
        as you pull further, creating natural rubber band behavior.
        ...
        result = (1.0 - (1.0 / ((x * self.rubber_band_coeff / dimension) + 1.0))) * dimension
        return sign * result
    ...
    def update_velocity(self, dt):
        ...
        if abs(self.velocity) <= self.min_velocity and abs(self.overscroll) < self.min_overscroll:
            self.velocity = 0
            self.overscroll = 0
            return
    ...
    def _apply_spring_force(self, dt):
        '''Apply critically damped spring force for stable bounce-back.
        
        Uses the critical damping formula to ensure no oscillation:
        damping = 2 * sqrt(stiffness * mass)
        
        This is what makes the effect stable - critical damping prevents overshoot.
        '''
    ...
    def apply_distance(self, distance):
        ...
        if self.is_manual and abs(self.overscroll) > 0:
            ...
            current_factor = abs(self._rubber_band_clamp(self.overscroll, dimension) / 
        ...
        super(FlutterScrollEffect, self).apply_distance(distance)
